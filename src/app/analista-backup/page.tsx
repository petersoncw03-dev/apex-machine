...
import { BrainCircuit, Bell, Volume2, VolumeX, Search, Zap, X, ChevronRight, List, RefreshCw } from "lucide-react";
...
export default function AnalistaPage() {
  ...
  const formatPatternList = () => {
    return [...discovered].sort((a, b) => {
      ...
      if (sortBy === 'PA') return ((b as any).pa || 0) - ((a as any).pa || 0);
  ...
  const generateWildcardVariations = (baseElements: PatternElement[], useWildcards: boolean, maxWildcards: number): PatternElement[][] => {
    ...
    const generate = (current: PatternElement[], index: number, wildcardsUsed: number) => {
        ...
        if (wildcardsUsed < maxWildcards && index > 0 && el.t === 'c') {
            if (el.v !== 'B') {
...
const runMixedDiscoveryEntrada = (config: any, currentData: Roll[], isAuto: boolean, oldActiveIds: Set<string> = new Set()) => {
  ...
  const runLightUpdate = (currentData: Roll[], oldActiveIds: Set<string>, isManual: boolean = false) => {
     ...
     if (!isManual && lastScoreboardRollId.current !== latestRoll.id) {
         let anyActive = false;
         let anyWin = false;
         ...
         for (const pat of discovered) {
             if (pat.activeNow) {
                 anyActive = true;
                 if (evaluateHit(latestRoll, pat.target, coverWhite)) {
                     anyWin = true;
                 }
                 if (Number(latestRoll.roll) === 0 || getCol(latestRoll) === 'B') {
                     anyWhiteWin = true;
                 }
             }
         }
...
const runFullDiscovery = (config: any, currentData: Roll[], isAuto: boolean, oldActiveIds: Set<string> = new Set()) => {
  ...
  const handleProcessIAClick = () => {
    ...
    lastScoreboardRollId.current = null;
  ...
  const applyTrend = (result: TrendResult) => {
    ...
    setTimeout(() => {
       const config = {
          periodHours: trendHours, patternType: 'TODOS', entriesLimit: result.bestEntries, targetFocus: result.target || trendTarget, coverWhite, continuousRead: false, useWildcards, maxWildcards,
          minTriggers: result.bestPatternSize > 5 ? 2 : 3, minWinRate: Math.max(wr - 5, 75), maxSa: 5, minSaFilter: 0, minPaFilter: 0, sizeRange: [result.bestPatternSize, result.bestPatternSize]
       };
       setAppliedFilters(config);
       runFullDiscoveryCiclo(config, data, false);
    }, 300);
  };
      ...
      <div className="bg-[#0a0a0f] border-b border-white/5 p-4 flex items-center justify-between shadow-2xl z-40">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4 border-r border-white/10 pr-6">
            <Link href="/analista2" className="text-xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500 flex items-center gap-2 hover:scale-105 transition-transform">
          ...
          {discovered.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between bg-[#12141c] p-4 rounded-xl border border-white/5">
                ...
                {/* GLOBAL SCOREBOARD */}
                <div className="flex items-center gap-4 bg-black/40 px-6 py-2 rounded-lg border border-white/5">
                  <div className="flex items-center gap-2">
