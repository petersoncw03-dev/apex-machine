...
function GeradorSaaSPageInner() {
  ...
  useEffect(() => {
    ...
    if (p.get('selectedSize'))   setSelectedSize(Number(p.get('selectedSize')));
  ...
  const savePreset = () => {
    ...
    const config = { periodHours, patternType, entriesRange, targetFocus, minTriggers, minWinRate, maxSa, minSaFilter, selectedSize, initialStake, martingaleMultiplier };
  ...
  const handleSaveAllToVPS = async () => {
    ...
    const filters = { periodHours, patternType, entriesRange, targetFocus, minTriggers, minWinRate, maxSa, minSaFilter, selectedSize, initialStake, martingaleMultiplier, useTrendFilter, ind1Type, ind1Period, ind2Type, ind2Period, useMixedMining };
...
;
  ...
  useEffect(() => {
    workerRef.current = new Worker(new URL('../../workers/discovery.worker.ts', import.meta.url));
  ...
  useEffect(() => {
    if (workerRef.current) {
      workerRef.current.onmessage = (e) => {
         ...
         if (results) setDiscovered(results);
         setIsDiscovering(false);
         setIsMixedMining(false);
      };
    }
  }, []);

  const runWorker = (action: string, config: any, currentData: Roll[], isAuto: boolean, oldActiveIds: Set<string> = new Set(), mode: string = 'full') => {
      if (!workerRef.current) return;
      if (!isAuto && mode === 'full') setIsDiscovering(true);
      if (!isAuto && mode === 'mixed') setIsMixedMining(true);
      workerRef.current.postMessage({
          action,
          config,
          currentData,
          isAuto,
          oldActiveIdsArray: Array.from(oldActiveIds),
          mode
      });
  };

  const runFullDiscovery = (config: any, currentData: Roll[], isAuto: boolean, oldActiveIds: Set<string> = new Set()) => {
      if (config.lossMode === 'ENTRADA') runWorker('runFullDiscoveryEntrada', config, currentData, isAuto, oldActiveIds, 'full');
      else runWorker('runFullDiscoveryCiclo', config, currentData, isAuto, oldActiveIds, 'full');
  };

  const runMixedDiscovery = (config: any, currentData: Roll[], isAuto: boolean, oldActiveIds: Set<string> = new Set()) => {
      if (config.lossMode === 'ENTRADA') runWorker('runMixedDiscoveryEntrada', config, currentData, isAuto, oldActiveIds, 'mixed');
      else runWorker('runMixedDiscoveryCiclo', config, currentData, isAuto, oldActiveIds, 'mixed');
  };
        ...
        <aside className="w-80 bg-[#0a0a0f] border-r border-white/5 flex flex-col shadow-2xl overflow-y-auto custom-scrollbar p-6 gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1 mt-2">
              ...
              <div className="flex items-center bg-[#12141c] rounded-md p-1 border border-white/10 relative">
                <div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-[#8b008b] rounded-sm transition-all duration-300 shadow-[0_0_10px_rgba(139,0,139,0.3)] ${lossMode === 'CICLO' ? 'left-1' : 'left-[calc(50%+2px)]'}`}></div>
                <button onClick={() => setLossMode('CICLO')} className={`flex-1 text-[9px] font-black uppercase tracking-widest py-1.5 relative z-10 transition-colors ${lossMode === 'CICLO' ? 'text-white' : 'text-gray-500 hover:text-white'}`}>Por Ciclo</button>
                <button onClick={() => setLossMode('ENTRADA')} className={`flex-1 text-[9px] font-black uppercase tracking-widest py-1.5 relative z-10 transition-colors ${lossMode === 'ENTRADA' ? 'text-white' : 'text-gray-500 hover:text-white'}`}>Por Entrada</button>
              </div>
            </div>
