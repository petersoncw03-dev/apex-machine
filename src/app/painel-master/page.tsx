...
export default function RadarAvancado() {
  ...
  useEffect(() => {
    ...
    for (let i = globalData.length - 1; i >= 0; i--) {
      if (isW(globalData[i])) {
        rodadasAtrasW = globalData.length - 1 - i;
        minAtrasW = Math.floor((now - new Date(globalData[i].timestamp).getTime()) / 60000);
        break;
      }
    }
    ...
    const findSeqW = (count: number) => {
      let maxDelay = 0;
      let lastEndIdx = -1;

      // Calculate max delay across ENTIRE globalData (to support > 72h delays)
      for (let i = 0; i <= globalData.length - count; i++) {
        let isSeq = true;
        for (let j = 0; j < count; j++) {
          if (!isW(globalData[i + j])) {
            isSeq = false;
            break;
          }
        }
      ...
      // Iterate Newest to Oldest to find the FIRST sequence going backwards
      for (let i = globalData.length - 1; i >= count - 1; i--) {
        ...
        if (isSeq) {
          ...
          // Calculate how many brancos since this sequence ended
          brancosSem = 0;
          for (let k = i + 1; k < globalData.length; k++) {
            if (isW(globalData[k])) {
              brancosSem++;
            }
          }
      ...
      if (rodadasAtras === -1) {
        brancosSem = 0;
        for (let k = 0; k < globalData.length; k++) {
          if (isW(globalData[k])) brancosSem++;
        }
      }
    ...
    const ds = findSeqW(2);
    const ts: any = findSeqW(3);

    // Calculate Duplos without Triplos
    let maxDuplosBetweenTriplos = 0;
    let currentDuplos = 0;
    let idx = 0;
    while (idx <= globalData.length - 2) {
      if (idx <= globalData.length - 3 && isW(globalData[idx]) && isW(globalData[idx + 1]) && isW(globalData[idx + 2])) {
        // Triplo
        if (currentDuplos > maxDuplosBetweenTriplos) maxDuplosBetweenTriplos = currentDuplos;
        currentDuplos = 0;
        idx += 3;
      } else if (isW(globalData[idx]) && isW(globalData[idx + 1])) {
        // Duplo
        currentDuplos++;
        idx += 2;
      } else {
  ...
  const iaStats = useMemo(() => {
    if (!globalData || globalData.length < 100) return { sa: 0, sm: 0 };

    const getMinuteStamp = (time: number) => Math.floor(time / 60000);

    const startTime = new Date(globalData[0].timestamp).getTime();
    const endTime = new Date(globalData[globalData.length - 1].timestamp).getTime();
  ...
  useEffect(() => {
    const measure = () => {
      if (tickerRef.current) {
        const w = tickerRef.current.clientWidth;
        // w-[56px] + gap-2 (8px) = 64px.
        setTickerCols(Math.max(10, Math.floor((w - 16) / 64)));
      }
    };
    measure();
  ...
  const isBranco = (r: any) => r?.color?.toUpperCase() === 'BRANCO' || r?.color?.toUpperCase() === 'B' || Number(r?.roll) === 0;
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar w-full z-10 flex flex-col relative">
        ...
        <div className="w-full max-w-[1600px] mx-auto px-6 pt-6 pb-2 shrink-0 flex flex-col gap-4 z-50">
           ...
           <div className="bg-[#0f141e]/80 backdrop-blur-xl border border-[#00c83a]/25 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 relative flex flex-col">
             ...
             <div 
               ref={tickerRef} 
               className="w-full flex flex-row-reverse gap-3 overflow-hidden justify-start items-center p-6 bg-black/40"
             >
             ...
             {analiseBrancos.rodadasAtras >= 20 && (
               <div className="mx-5 mb-5 px-4 py-2 bg-gradient-to-r from-amber-500/20 to-transparent border border-amber-500/30 rounded-lg flex justify-between items-center shadow-[0_0_15px_rgba(245,158,11,0.2)]">
                 <div className="flex items-center gap-2 text-amber-500">
                   <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
                   <span className="text-[11px] font-black tracking-widest">ALERTA</span>
                   <span className="text-amber-200/70 text-[11px] border-l border-amber-500/30 pl-2 ml-1">Possível Recuperação</span>
                 </div>
            ...
            <div className="flex flex-col xl:flex-row gap-6 pb-32 items-stretch">
              ...
              {activeTab === 'home' && (
                ...
                <div className="bg-[#0f141e]/80 backdrop-blur-xl border border-[#00c83a]/25 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 relative">
                  ...
                  <div className="p-4 flex flex-col gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-transparent flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(255,255,255,0.2)] overflow-hidden">
                ...
                <div className="bg-[#0f141e]/80 backdrop-blur-xl border border-[#00c83a]/25 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 relative">
                  ...
                  <div className="p-4 flex flex-col gap-4">
                    {/* Preto */}
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-[#2C2F33] flex items-center justify-center shrink-0 border-[1.5px] border-white/40 shadow-[0_0_15px_rgba(44,47,51,0.5)]"></div>
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                      <div className="w-10 h-10 rounded-lg bg-[#E51E3E] flex items-center justify-center shrink-0 border-[1.5px] border-white/80 shadow-[0_0_15px_rgba(229,30,62,0.4)]"></div>
                      <div className="flex flex-col justify-center gap-0.5">
                        <div className="flex items-center gap-3">
                ...
                <div className="bg-[#0f141e]/80 backdrop-blur-xl border border-[#00c83a]/25 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 relative">
                  ...
                  <div className="p-4 flex flex-col gap-4">
                    {/* Par */}
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(6,182,212,0.4)] border border-cyan-400/50">
                    ...
                    {/* Ímpar */}
                    <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(99,102,241,0.4)] border border-indigo-400/50">
              ...
              <div className="flex flex-col gap-6 overflow-hidden h-[870px] max-h-[870px] shrink-0">
                <div className={`${CARD_GREEN} flex flex-col h-full`}>
                  ...
                  <div className="px-3 py-2 bg-[#050507] border-b border-white/10 flex items-center justify-between gap-2">
                  ...
                  <div className="flex flex-col border border-white/5 rounded-lg h-full overflow-hidden mx-3 mb-3">
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#131722] relative">
                      <div className="grid grid-cols-3 text-center bg-[#0b0e14] border-b border-white/5 sticky top-0 z-20">
                        <div className="py-1.5 text-[12px] font-bold text-slate-400 border-r border-white/5">Hora</div>
                        <div onClick={() => handleScanSort('SA')} className="py-1.5 text-[12px] font-bold text-slate-400 border-r border-white/5 cursor-pointer hover:bg-[#131722] transition-colors">SA{scanSortIcon('SA')}</div>
                        <div onClick={() => handleScanSort('SM')} className="py-1.5 text-[12px] font-bold text-slate-400 cursor-pointer hover:bg-[#131722] transition-colors">SM{scanSortIcon('SM')}</div>
                      </div>
                      ...
                      {(() => {
                        ...
                        return scannerStats.map(s => {
                          ...
                          return (
                            <div key={`${s.id}-${s.hits}`} className={`grid grid-cols-3 text-center border-b border-white/5 text-[13px] font-mono font-bold transition-colors ${isCurrent ? "ring-2 ring-inset ring-amber-500 bg-[#f59e0b]/20 z-10 relative" : "hover:bg-slate-700/20"}`}>
                  ...
                  <div className="flex flex-col border border-white/5 rounded-lg h-full overflow-hidden mx-3 mb-3">
                    <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#131722] relative">
                      <div className="grid grid-cols-4 text-center bg-[#0b0e14] border-b border-white/5 sticky top-0 z-20">
                        <div className="py-1 text-[10px] font-black uppercase text-slate-400 tracking-widest border-r border-white/5">#</div>
                        <div onClick={() => handleSomaSort('BCO')} className="py-1 text-[10px] font-black uppercase text-slate-400 tracking-widest border-r border-white/5 cursor-pointer hover:bg-[#131722] transition-colors">BCO{sortIcon('BCO')}</div>
                        <div onClick={() => handleSomaSort('SM')} className="py-1 text-[10px] font-black uppercase text-slate-400 tracking-widest border-r border-white/5 cursor-pointer hover:bg-[#131722] transition-colors">SM{sortIcon('SM')}</div>
                        <div onClick={() => handleSomaSort('SA')} className="py-1 text-[10px] font-black uppercase text-slate-400 tracking-widest cursor-pointer hover:bg-[#131722] transition-colors">SA{sortIcon('SA')}</div>
                      </div>
                      ...
                      {(() => {
                        ...
                        return somaStats.map((s, i) => {
                          ...
                          return (
                            <div key={i} className={`grid grid-cols-4 text-center border-b border-white/5 text-[12px] font-mono transition-colors group ${isCurrent ? "ring-2 ring-inset ring-amber-500 bg-[#f59e0b]/20 z-10 relative" : "hover:bg-slate-700/30"}`}>
            ...
            <div className="flex-1 flex flex-col gap-6 min-w-0">
                ...
                <div className="overflow-x-auto p-4 bg-transparent">
                  <div className="flex flex-col min-w-[700px] border border-white/5 rounded-lg overflow-hidden bg-[#131722] shadow-lg">
                    <div className="flex bg-[#0b0e14] border-b border-white/5">
                      <div className="w-28 shrink-0 py-3 border-r border-white/5 text-[11px] font-black uppercase text-center text-slate-400 flex items-center justify-center">Período</div>
              ...
              <div className={`${CARD} shrink-0`}>
                ...
                <div className="border-t border-white/10 p-4 bg-transparent">
                  <h3 className="text-[11px] font-black uppercase tracking-widest text-white mb-3 text-center">Os 3 Melhores (Puxam Branco)</h3>
            ...
            <div className="flex justify-between items-center p-4 border-b border-[#1c1c24] bg-[#13141a]">
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleOpenMaximas(30)} className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded transition-colors ${maximasDays === 30 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10 hover:text-white'}`}>30d</button>
                  <button onClick={() => handleOpenMaximas(60)} className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded transition-colors ${maximasDays === 60 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10 hover:text-white'}`}>60d</button>
                  <button onClick={() => handleOpenMaximas(90)} className={`px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded transition-colors ${maximasDays === 90 ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10 hover:text-white'}`}>90d</button>
                </div>
              </div>
...
function HistoryPanel({ playAlert, globalData, histRealTime, setHistRealTime, histFixedCols, setHistFixedCols, histReverse, setHistReverse, histShowSeconds, setHistShowSeconds }: any) {
  ...
  const renderStoneUI = (stone: any) => {
     ...
     if (stone.type === 'number') content = <GlobalStoneIcon n={stone.val} size="md" />;
        ...
        if (stone.val === 'ANY') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#008ce3] shadow-sm"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1.5px] border-white/80"></div></div>;
        if (stone.val === 'V') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#E51E3E] shadow-sm"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1.5px] border-white/80"></div></div>;
        if (stone.val === 'P') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#2C2F33] shadow-sm"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1px] border-white/40"></div></div>;
        if (stone.val === 'B') content = <GlobalStoneIcon n={0} size="md" />;
     }
     else if (stone.type === 'dual') {
        if (stone.val === 'VP') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm relative overflow-hidden"><div className="w-1/2 h-full bg-[#E51E3E]"></div><div className="w-1/2 h-full bg-[#2C2F33]"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-white/80 shadow-[0_0_2px_rgba(0,0,0,0.5)]"></div></div>;
        if (stone.val === 'BP') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm relative overflow-hidden border border-[#2C2F33]/20 bg-white"><div className="w-1/2 h-full bg-white"></div><div className="w-1/2 h-full bg-[#2C2F33]"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-[#2C2F33]/30 shadow-[0_0_2px_rgba(0,0,0,0.2)]"></div></div>;
        if (stone.val === 'BV') content = <div className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm relative overflow-hidden border border-[#E51E3E]/20 bg-white"><div className="w-1/2 h-full bg-white"></div><div className="w-1/2 h-full bg-[#E51E3E]"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-[#E51E3E]/50 shadow-[0_0_2px_rgba(0,0,0,0.1)]"></div></div>;
     }
     return (
            ...
            {(activeTool === 'notificador' || activeTool === 'validador') && (
               <div className="flex flex-col gap-6 w-full animate-fade-in">
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-2">
                     ...
                     <button onClick={handleAddPattern} className="flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 hover:text-white transition-all shrink-0 ml-1" title="Criar novo padrão">
                  ...
                  <div className="flex gap-6 items-stretch w-full">
                     ...
                     <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-3">
                           ...
                           <div className="flex flex-wrap gap-2">
                              {/* Azul (Qualquer) */}
                              <button onClick={() => addStoneToPattern({type:'color',val:'ANY'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#008ce3] shadow-sm hover:scale-105 active:scale-95 transition-transform"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1.5px] border-white/80 pointer-events-none"></div></button>
                              {/* Vermelho */}
                              <button onClick={() => addStoneToPattern({type:'color',val:'V'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#E51E3E] shadow-sm hover:scale-105 active:scale-95 transition-transform"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1.5px] border-white/80 pointer-events-none"></div></button>
                              {/* Preto */}
                              <button onClick={() => addStoneToPattern({type:'color',val:'P'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 bg-[#2C2F33] shadow-sm hover:scale-105 active:scale-95 transition-transform"><div className="rounded-full flex items-center justify-center w-7 h-7 border-[1px] border-white/40 pointer-events-none"></div></button>
                              {/* Branco / Blaze */}
                              ...
                              {/* Vermelho / Preto */}
                              <button onClick={() => addStoneToPattern({type:'dual',val:'VP'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm hover:scale-105 active:scale-95 transition-transform relative overflow-hidden"><div className="w-1/2 h-full bg-[#E51E3E] pointer-events-none"></div><div className="w-1/2 h-full bg-[#2C2F33] pointer-events-none"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-white/80 shadow-[0_0_2px_rgba(0,0,0,0.5)] pointer-events-none"></div></button>
                              {/* Branco / Preto */}
                              <button onClick={() => addStoneToPattern({type:'dual',val:'BP'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm hover:scale-105 active:scale-95 transition-transform relative overflow-hidden border border-[#2C2F33]/20 bg-white"><div className="w-1/2 h-full bg-white pointer-events-none"></div><div className="w-1/2 h-full bg-[#2C2F33] pointer-events-none"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-[#2C2F33]/30 shadow-[0_0_2px_rgba(0,0,0,0.2)] pointer-events-none"></div></button>
                              {/* Branco / Vermelho */}
                              <button onClick={() => addStoneToPattern({type:'dual',val:'BV'})} className="rounded flex items-center justify-center shrink-0 w-10 h-10 shadow-sm hover:scale-105 active:scale-95 transition-transform relative overflow-hidden border border-[#E51E3E]/20 bg-white"><div className="w-1/2 h-full bg-white pointer-events-none"></div><div className="w-1/2 h-full bg-[#E51E3E] pointer-events-none"></div><div className="absolute rounded-full w-7 h-7 border-[1.5px] border-[#E51E3E]/50 shadow-[0_0_2px_rgba(0,0,0,0.1)] pointer-events-none"></div></button>
                           </div>
                        </div>
...
function HistoryCardComponent({ roll, showSec }: any) {
   ...
   return (
      <div className="flex flex-col items-center shrink-0 w-[54px] p-1 gap-[3px] bg-[#0b0e14]/70 rounded-lg border border-[#00c83a]/20 shadow-sm transition-all hover:bg-[#00c83a]/10">
         ...
         <div className="flex justify-center w-full">
            <div className={`text-center py-[2px] px-1.5 whitespace-nowrap text-[9px] font-black tracking-widest leading-none rounded bg-transparent text-slate-400`}>
               {timeStr}
            </div>
         </div>
      </div>
